				...
				if($password == $confirmPassword)
				{
					...
					if($user_found == false)
					{
						...
						if ($run) {
							...
							$query2 = "INSERT INTO login(`id`, `phoneNumber`, `password`) VALUES ($rider_id,'$phoneNumber','$password')";
								// $run will check the query and the connection of DB
							$run1 = mysqli_query($con, $query2);
								// This condition will check and validate all variables and query 
							if ($run1) {
								$query3 = "INSERT INTO rider(`rider_id`, `name`, `phoneNumber`, `email`, `DOB`, `profilePicture`, `rating`, `ratingInWords`) VALUES ('$rider_id', '$name', '$phoneNumber', '$email', '$DOB' , '$profilePicture', '$rating', '$ratingInWords')";
								// $run will check the query and the connection of DB
								...
								header("location: signupRider.php?success=".urldecode("Error"));
						}
					}
				}
				else 
...
<body>
	<div class="signup">
		...
		<form action="signupRider.php" method="post" enctype="multipart/form-data">
			...
			<p> Already have an Account. <a href="loginRider.php" style="color:orange;"> Just Login. </a> <!-- link to html -->
			</form>
		</div>
	</body>
	</html>